...
<Callout type="warn" title="Why this isn't automated in the SDK">
  ...
  `@lumierelabs/daraja` would mean either shipping a certificate that silently
  goes stale, or adding an update mechanism that fetches and trusts a
  certificate at runtime, which is a meaningfully different (and riskier)
  security posture than a one-time offline step you control yourself.
</Callout>
