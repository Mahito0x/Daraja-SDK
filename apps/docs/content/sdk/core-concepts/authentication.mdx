...
description: How OAuth token caching and replacement work under the hood.
icon: KeyRound
---
...
```ts
// Every endpoint method calls this for you.
...
5. Once the buffer is crossed, the next call transparently fetches a new token and replaces the cache. This is a new client-credentials request, not an OAuth refresh-token flow.
